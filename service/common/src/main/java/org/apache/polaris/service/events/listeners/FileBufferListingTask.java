...
class FileBufferListingTask implements Runnable {
    ...
    public FileBufferListingTask(String bufferDirectory, Function<TaskSubmissionInput, Future> taskSubmitter, ConcurrentHashMap<String, Future> activeFlushFutures, int timeToFlush, BiConsumer<String, Integer> rotateShard, BiConsumer<String, List<PolarisEvent>> eventWriter) {
    ...
    public void run() {
        ...
        try {
            ...
            if (realmDirs == null || realmDirs.length == 0) {
                LOGGER.debug("No event buffer files found in path: {}", topLevelDirectory.getAbsolutePath());
                return;
            }
            for (File realmDir : realmDirs) {
                ...
                for (File bufferDir : bufferDirs) {
                    ...
                    Arrays.stream(bufferFiles).map(File::getAbsolutePath).filter(file -> !file.equals(latestFile.getAbsolutePath()) && !activeFlushFutures.containsKey(file)).forEach(filesToFlush::add);
                    ...
                    if (System.currentTimeMillis() - Long.parseLong(fileDeconstructed[fileDeconstructed.length - 1]) > timeToFlush) {
                        ...
                        int bufferNum = Integer.parseInt(shardDirNameDeconstructed[shardDirNameDeconstructed.length - 1]);
                ...
                for (String file : filesToFlush) {
                    FileFlushTask task = new FileFlushTask(file, realmDir.getName(), activeFlushFutures::remove, this.eventWriter);
                    activeFlushFutures.computeIfAbsent(file, k -> taskSubmitter.apply(new TaskSubmissionInput(task, 0)));
                }
            }
        } finally {
